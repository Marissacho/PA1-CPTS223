//PA1
#include <iostream>
/*
ADVANTAGES/DISADVANTAGES LINKED LIST
1.advantage:allows efficient storage that makes it easier to handle the list that frequently gets bigger and smaller
2.disadvantage:slower access time
ADVANTAGES/DISADVANTAGES ARRAY
1.advantage:can access items faster and more efficiently
2.disadvantage:arrays have a fixed size making it harder if you need to add more and delete items
*/
#include <iostream>
#include <string>
#include "linkedlist.h"
using namespace std;
int main()
{
linked_list<string,string> c_list;
string userName, newCommandToAdd, oldCommandTodelete,newDescription;
int selection;
while (true)
{
cout << "Please select an option listed below:\n";
cout << "1. Game Rules\n2. Play Game\n3. Load Previous Game\n4. Add Command\n5. Remove Command\n6. Exit\n";
cout << "Your selection: ";
cin >> selection;
if (cin.fail()) // the input is not an integer
{
cout << "Your selection is invalid. Please select again." << endl;
cout <<
"=============================================================================" <<
endl << endl;
cin.clear();
cin.ignore();
}
else
{
switch (selection)
{
case 1: // Game Rules
{
cout << "================================= Game Rule ===================================" << endl;
cout << "To play the game, select \"2\" where you will be prompted for your name and number of questions.\n";
cout << "Each question presents a specific Linux command where you will be able to choose from 3 different options. Each correct answer yields a point.\n";
cout << "You can also add and delete commands as needed.\n";
cout << "================================= Game Rule ===================================" << endl << endl;
break;
}
case 2:
cout<<"Please enter your name:";
cin>>userName;
c_list.playGame();
case 3:
{
cout << "Please enter your name: ";
cin >> userName;
c_list.outfile("profiles.csv");
break;
}
case 4: // Add Command
{
cout << "To add a command to the library, please enter the command name that you would add: ";
cin >> newCommandToAdd;
cout<<"enter the description for the command: ";
cin.ignore();
getline(cin,newDescription);
c_list.insertAtFront(newCommandToAdd, newDescription);
cout<<"Command added"<<endl;
break;
}
case 5: // Remove Command
{
cout << "Please enter the name of the command that you would remove: ";
cin >> oldCommandTodelete;
c_list.removeCommand(oldCommandTodelete);
cout<<"Command removed"<<endl;
break;
}
case 6: // Exit
{
c_list.outfile("commands.csv");
return 0;
}
} // end of switch(selection)
} // end of else: check (cin.fail())
} // end of while(true)
} // end of int main()
